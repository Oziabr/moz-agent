(() => {
  ...
  var renderState = (domain, state) => {
    ...
    els.status.textContent = state.enabled ? state.allowWrite ? "enabled, can submit forms" : "enabled, read-only" : "disabled";
  };
  var onEnableChange = (domain) => async () => {
  ...
  var onWriteChange = (domain) => async () => {
