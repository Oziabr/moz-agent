...
${suffix}`;
  ...
  function loadOtel() {
    if (otelModulePromise === null) otelModulePromise = Promise.resolve(null).catch(() => null);
  ...
  var TABLE_DOMAINS = "moz_agent_enabled_domains";
  var TABLE_JOBS = "moz_agent_jobs";
  var BADGE_COLOR_OFF = "#888780";
  var BADGE_COLOR_READ = "#4A90D9";
  ...
  var hasHostPermission = (domain) => browser.permissions.contains({ origins: [`*://${domain}/*`] });
  var requestHostPermission = (domain) => browser.permissions.request({ origins: [`*://${domain}/*`] });
  var removeHostPermission = (domain) => browser.permissions.remove({ origins: [`*://${domain}/*`] });
  var badgeForState = (state, jobCount) => {
  ...
  var setBadgeForTab = async (tabId, domain) => {
  ...
  var refreshActiveTabBadge = async () => {
  ...
  var loadDomainState = async () => {
    ...
    grantChecks.filter((check) => !check.granted).forEach((check) => {
  ...
  var setDomainEnabled = async (domain, enabled) => {
    ...
    const { error } = await supabase.from(TABLE_DOMAINS).upsert(
  ...
  var loadDomainStateAndRefresh = async () => {
  ...
  var handleMessage = (message, sender, sendResponse) => {
    ...
    if (message.type === "setEnabled") {
      setDomainEnabled(message.domain, message.enabled).then(sendResponse).catch((err) => sendResponse({ ok: false, reason: err.message }));
      return true;
    }
